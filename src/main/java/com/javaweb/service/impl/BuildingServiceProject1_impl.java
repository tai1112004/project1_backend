...
public class BuildingServiceProject1_impl implements BuildingServiceProject1 {
	...
	@Override
	public List<BuildingDTO_request> data_request(Map<String,Object> param ) {
		System.out.println("Service") ; 
		List<BuildingEntityProject1> source = repository.FindRequest(param ) ;  
		List<BuildingDTO_request> front = new ArrayList<>() ; 
		
		for(BuildingEntityProject1 rs:source)
		{
			BuildingDTO_request data = model.map(rs,BuildingDTO_request.class) ; 
//			data.setId(rs.getId());
